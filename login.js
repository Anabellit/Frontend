    ...
    $.ajax({
        ...
        .done(function (response, status, xhr) {
            ...
            if (token) {
                ...
                // Da die Rollen im JWT als Liste gespeichert sind, holen wir die erste Rolle
                var roles = decodedToken.roles;  // Claim für Rollen (eine Liste)
                var role = roles && roles.length > 0 ? roles[0] : null;  // Erste Rolle extrahieren

                if (role) {
                    // Weiterleitung basierend auf der Rolle
                    if (role === "ADMIN") {
                        window.location.href = "admin-dashboard.html";  // Weiterleitung zur Admin-Seite
                    } else if (role === "USER") {
                        window.location.href = "search-accommodation.html";  // Weiterleitung zur User-Seite
                    } else {
                        alert("Unbekannte Rolle: " + role);
                    }
                } else {
                    alert("Keine Rolle im Token gefunden.");
                }
            } else {
